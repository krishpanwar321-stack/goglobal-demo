import {
  ...
  export default function WhyGoGlobal() {
    return (
      <section className="bg-[#0F172A] px-5 py-16 text-white sm:px-8 lg:px-10 lg:py-20">
  
        {/* HEADER */}
  
        <div className="max-w-3xl">
  
          <p className="text-sm font-medium uppercase tracking-[0.25em] text-[#60A5FA]">
            Why GoGlobal
          </p>
        ...
        <div className="mt-10 grid gap-5 sm:mt-14 sm:gap-6 md:grid-cols-2">
          ...
          {features.map((item) => {
            ...
            return (
              <div
                key={item.title}
                className="group rounded-[28px] border border-white/10 bg-white/[0.04] p-6 backdrop-blur-md transition-all duration-300 hover:-translate-y-1 hover:bg-[#1E293B] hover:border-[#3B82F6]/30 hover:shadow-blue-900/20 hover:shadow-2xl sm:p-7"
              >
  
                {/* ICON */}
  
                <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-[#1E293B] sm:h-14 sm:w-14">
                ...
                <p className="mt-4 max-w-lg text-[15px] leading-7 text-[#CBD5E1] sm:mt-5 sm:text-[16px]">
